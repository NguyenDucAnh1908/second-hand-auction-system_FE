...
export default function ListOfSellerProductPage() {
    ...
    const handleOpenModal = (auction) => {
        setSelectedAuction(auction); // Lưu thông tin đấu giá vào state
    ...
    const renderTableRows = () => {
        if (isLoading) return <tr><td colSpan={7}>Loading...</td></tr>;
        if (error) return <tr><td colSpan={7}>Error fetching data</td></tr>;
                            ...
                            <div className="mt-4 flex flex-col items-end">
                                <div className="w-[100%] md:w-full md:px-5">
                                    <div className="flex items-start">
                                        <div className="flex-1 self-center rounded-[16px] bg-[url(/images/img_group_46876.png)] bg-cover bg-no-repeat pt-[84px] md:h-auto md:py-5">
                                            <div className="flex flex-col gap-2">
                                                <div className="ml-1 flex md:ml-0">
                                                    <Heading size="headingxl" as="h1" className="text-[30px] font-semibold uppercase text-gray-900_01 md:text-[44px] sm:text-[38px] -mt-[80px] ml-[20px]">
                                                        Danh sách sản phẩm
                                                    </Heading>
                                                </div>
                                                <div className="mr-[38px] flex justify-between gap-5 md:mx-0 sm:flex-col -mt-[50px] ml-[15px]">
                                                    <InputDH
                                                        ...
                                                        suffix={searchBarValue1?.length > 0 ? (
                                                            <CloseSVG onClick={() => setSearchBarValue1("")} height={18} width={18} fillColor="#626974ff" />
                                                        ) : (
            ...
            {selectedAuction && (
                <div className="space-y-4 p-4">
                    <div className="bg-gray-100 p-4 rounded-lg shadow">
                        <h4 className="text-lg font-bold">Mã sản phẩm: <span className="text-blue-600">{selectedAuction.auction_id}</span></h4>
                        <p className="text-gray-700">Người thẩm định: <span className="font-semibold">{selectedAuction.created_by}</span></p>
                        <p className="text-gray-700">Giá khởi điểm: <span className="font-semibold">{selectedAuction.start_price.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span></p>
                        <p className="text-gray-700">Thời gian đấu giá: <span className="font-semibold">{selectedAuction.start_time} - {selectedAuction.end_time}</span></p>
                        <p className="text-gray-700">Ngày đấu giá: <span className="font-semibold">{formatDate(selectedAuction.startDate)} - {formatDate(selectedAuction.endDate)}</span></p>
                        <p className="text-gray-700">Ngày thẩm định sản phẩm: <span className="font-semibold">{formatDate(selectedAuction.approved_at)}</span></p>
                        <p className="text-gray-700">Trạng thái phiên đấu giá: <span className="font-semibold">{selectedAuction.status}</span></p>
                    </div>
                </div>
            )}
        </Modal>
