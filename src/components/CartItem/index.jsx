...
export default function CartItem({ product, refetchItem }) {
  ...
  const auctionStartDate = product.auction?.startDate || null;
  ...
  if (auctionEndDate && auctionEndTime) {
    try {
      ...
      const dateStart = `${auctionStartDate}T${auctionStartTime}`;
      ...
      <div className="m-4">
        ...
        <Badge.Ribbon
          text={
            Date.now() < startTime ? (
                // Auction not started
                <div className="bg-yellow-400 text-black px-3 py-2 rounded-md font-normal text-sm">
                    Phiên đấu giá sẽ mở vào {auctionStartDate} {auctionStartTime}
                </div>
            ) : (
                // Auction ongoing
                <Countdown
        ...
        color={Date.now() < startTime ? "#FFC107" : "#9DE0AD"} // Yellow for "Not started", Green for "Ongoing"
        style={{
            display: 'flex',
